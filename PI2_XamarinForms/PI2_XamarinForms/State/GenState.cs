...
{
    public class GenState : SearchState
    {
        ...
        public override IEnumerable<Suicides> SearchedList(string keyword, List<Suicides> suicides)
        {
            if (keyword.Length >= 1)
            {
                IEnumerable<Suicides> sugestion = suicides.Where(c => c.Generation.ToString().ToLower().Contains(keyword.ToLower()));
                return sugestion;
            }
